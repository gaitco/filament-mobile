...
- **The README shows the app now.** Its images had drifted a long way from the
  package: everything under `art/` predated 0.7.0's visual pass over the very
  screens it pictured, so the listing advertised a UI that no longer existed.
  All five screens are recaptured, and three images are new — the field-type
  gallery beside the supported-inputs table, the relation write flow beside the
  Relations section, and the panel's screens near the top. Regenerated by
  `scripts/capture-shots.sh` and `art/render.sh`, so the next release can redo
  them in one command instead of by hand.
